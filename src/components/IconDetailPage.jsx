...
export function IconDetailPage({
  ...
  const [activeTab, setActiveTab] = useState('react'); // 'react' | 'vue' | 'html' | 'nextjs' | 'css' | 'svg' | 'jsx' | 'cdn' | 'uri'
  const [copied, setCopied] = useState(false);
  const [copiedCli, setCopiedCli] = useState(false);
  const [copiedCdn, setCopiedCdn] = useState(false);
  const [isCopyModalOpen, setIsCopyModalOpen] = useState(false);
  const [copyFormat, setCopyFormat] = useState('png'); // 'svg' | 'png' | 'jpg' | 'webp' | 'avif' | 'ico'
  const [copySize, setCopySize] = useState(256);
  const [customSizeInput, setCustomSizeInput] = useState('256');

  const [isDownloadModalOpen, setIsDownloadModalOpen] = useState(false);
  const [downloadFormat, setDownloadFormat] = useState('svg'); // 'svg' | 'png' | 'jpg' | 'webp' | 'avif' | 'ico' | 'zip'
  ...
  const variantsList = useMemo(() => {
    ...
    // Standard ordered variants list
  ...
  // Extract primary dominant color from SVG
  ...
  // Code Generation for Multi-Framework Tabs
  const getActiveCode = () => {
  ...
  const handleCopyFormattedAsset = async (format, size) => {
    ...
    try {
      if (format === 'svg') {
        ...
        // Render image to HTML5 canvas and copy binary image directly to clipboard
  ...
  const relatedIcons = useMemo(() => {
    ...
    // Match icons that share ANY category
    ...
    // Fallback: if no same category icons found, return first 18 icons
      ...
      <div className="thesvg-main-grid">
        ...
        <div className="thesvg-left-col">
          ...
          <div className="thesvg-preview-card glass-panel">
            {/* Top Row: Favorite (Left) & Color Pill (Right) */}
        ...
        <div className="thesvg-right-col">
          ...
          <div className="thesvg-header-row">
            ...
            <div className="thesvg-header-actions">
              {/* Copy Format & Resolution Modal Trigger */}
              <div className="thesvg-dropdown-wrap" ref={copyModalRef}>
                ...
                {isCopyModalOpen && (
                  <div className="thesvg-copy-dialog-popover glass-panel">
                    ...
                    {/* Pixel Frame Resolution (for PNG / JPG / WebP / AVIF raster) */}
              ...
              {/* Download Format & Resolution Modal Trigger */}
          ...
          <div className="thesvg-code-box-container glass-panel">
            ...
            <div className="thesvg-code-tabs-bar">
              ...
              {/* Right Format Pills & Copy Button */}
